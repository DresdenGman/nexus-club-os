...
const dict = {
  en: {
    ...
    my_clubs: 'My Clubs & Activities',
    activities: 'Activities',
    create_activity: 'Create Activity',
    act_title: 'Title *',
    act_desc: 'Description *',
    act_contact: 'Contact Info *',
    act_link: 'Join Link (optional)',
    act_host: 'Host Club *',
    act_collab: 'Joint Clubs (optional)',
    act_needs_approval: 'Require approval to join',
    act_no_activities: 'No activities yet. Create one!',
    act_published: 'Activity published!',
    act_submitted: 'Submitted for approval',
    act_joined: 'Joined!',
    act_applied: 'Applied - awaiting approval',
  ...
  zh: {
    ...
    act_host: '主办社团 *',
    act_collab: '联合社团（选填）',
    act_needs_approval: '加入需要审批',
    act_no_activities: '暂无活动，快来创建一个！',
    act_published: '活动已发布！',
    act_submitted: '已提交，等待审批',
    act_joined: '已加入！',
    act_applied: '已申请，等待审批',
    about_club: '关于本社团',
    recent_act: '近期活动',
            ...
            {myActivities.map((a: any) => (
              <div key={a.id} className="bg-bg p-4 flex items-start">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-mono text-[9px] uppercase opacity-50">{a.primary_club?.name} · {a.my_role === 'initiator' ? 'Initiator' : 'Participant'} · {a.participant_count || 0} joined</p>
                </div>
                <div className="ml-2 shrink-0">
                  <span className={`font-mono text-[9px] px-2 py-0.5 border ${a.my_status === 'pending' ? 'border-accent text-accent' : 'border-line'}`}>
                    {a.my_status === 'pending' ? 'Pending' : 'Active'}
                  </span>
                </div>
              </div>
            ))}
